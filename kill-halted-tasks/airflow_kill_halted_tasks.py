"""
A maintenance workflow that you can deploy into Airflow to periodically kill off
tasks that are running in the background that don't correspond to a running task
in the DB.
...
def get_airflow_run_regex():
    """Airflow run regex"""
    return '.*run\s([\w_-]*)\s([\w_-]*)\s([\w:.-]*).*'
...
def parse_process_linux_string(line):
    ...
    airflow_run_regex_match = re.search(get_airflow_run_regex(), command)
    if DEBUG:
...
def kill_halted_tasks_function(**context):
    ...
    logging.info("send_process_killed_email:      " +
                 str(SEND_PROCESS_KILLED_EMAIL))
    logging.info("process_killed_email_subject:   " +
                 str(PROCESS_KILLED_EMAIL_SUBJECT))
    logging.info("process_killed_email_addresses: " +
                 str(PROCESS_KILLED_EMAIL_ADDRESSES))
    logging.info("enable_kill:                    " +
                 str(ENABLE_KILL))
    logging.info("debug:                          " +
                 str(DEBUG))
    ...
    logging.info("Filtering out: Empty Lines, Grep processes, and this DAGs "
                 "Run.")
    search_output_filtered = [line
    ...
    for line in search_output_filtered:
        ...
        execution_date_to_search_for = str(process["airflow_execution_date"])\
                                           .replace("T", " ") + "%"
        logging.info("Execution Date to Search For: " +
        ...
        if DEBUG:
            ...
            for dag in session.query(DagModel).all():
                logging.info("DEBUG: dag: " + str(dag) +
                             ", dag.is_active: " + str(dag.is_active))
            logging.info("")
        logging.info("Getting dag where DagModel.dag_id == '" +
        ...
        if DEBUG:
            ...
            logging.info("DEBUG: Listing All Relevant DAG Runs (With State: " +
                         str(dag_run_relevant_states) + "): ")
            for dag_run in session.query(DagRun).filter(
                    DagRun.state.in_(dag_run_relevant_states)).all():
                logging.info("DEBUG: dag_run: " + str(dag_run) +
                             ", dag_run.state: " + str(dag_run.state))
            logging.info("")
        logging.info("Getting dag_run where DagRun.dag_id == '" +
                     str(process["airflow_dag_id"]) +
                     "' AND DagRun.execution_date LIKE '" +
                     str(execution_date_to_search_for) + "'")
        ...
        if dag_run.state not in dag_run_states_required:
            kill_reason = ("DAG RUN was found to not be in the states '" +
        ...
        # Checking to make sure the TaskInstance is available and in a running
        # state
        if DEBUG:
            ti_relevant_states = ["queued",
                                  "running",
                                  "up_for_retry"]
            logging.info("DEBUG: Listing All Relevant TaskInstances (With "
                         "State: " + str(ti_relevant_states) +
                         "): ")
            for task_instance in session.query(TaskInstance)\
                    .filter(TaskInstance.state.in_(ti_relevant_states)).all():
                logging.info("DEBUG: task_instance: " +
                             str(task_instance) +
                             ", task_instance.state: " +
                             str(task_instance.state))
            logging.info("")
        logging.info("Getting task_instance where TaskInstance.dag_id == '" +
                     str(process["airflow_dag_id"]) +
                     "' AND TaskInstance.task_id == '" +
                     str(process["airflow_task_id"]) +
                     "' AND TaskInstance.execution_date LIKE '" +
                     str(execution_date_to_search_for) + "'")
        ...
        if task_instance is None:
            kill_reason = ("Task Instance was not found in metastore. "
        ...
        if task_instance.state not in task_instance_states_required:
            kill_reason = ("The TaskInstance was found to not be in the "
                           "states '" +
    ...
    if ENABLE_KILL:
        ...
        if processes_to_kill:
            ...
            context['ti'].xcom_push(key='kill_halted_tasks.processes_to_kill',
...
def branch_function(**context):
    """Branch condition"""
    logging.info("Deciding whether to send an email about "
                 "tasks that were killed by this DAG...")
    logging.info("SEND_PROCESS_KILLED_EMAIL: '" +
                 str(SEND_PROCESS_KILLED_EMAIL) +
                 "'")
    logging.info("PROCESS_KILLED_EMAIL_ADDRESSES: " +
    ...
    if not SEND_PROCESS_KILLED_EMAIL:
        logging.info("Skipping sending an email since "
                     "SEND_PROCESS_KILLED_EMAIL is set to false")
        # False = short circuit the dag and don't execute downstream tasks
        return False
    if not PROCESS_KILLED_EMAIL_ADDRESSES:
        logging.info("Skipping sending an email since "
                     "PROCESS_KILLED_EMAIL_ADDRESSES is empty")
        # False = short circuit the dag and don't execute downstream tasks
        return False

    processes_to_kill = (context['ti']
                         .xcom_pull(task_ids=KILL_HALTED_TASKS_OPR.task_id,
                                    key='kill_halted_tasks.processes_to_kill'))
    logging.info("processes_to_kill from xcom_pull: " + str(processes_to_kill))
    if processes_to_kill is not None and processes_to_kill:
        logging.info("There were processes to kill")
        if ENABLE_KILL:
            logging.info("enable_kill is set to true")
            logging.info("Opting to send an email to alert the "
                         "users that processes were killed")
    ...
    logging.info("Opting to skip sending an email since no processes were "
                 "killed")
    # False = short circuit the dag and don't execute downstream tasks
    return False


KILL_HALTED_TASKS_OPR = PythonOperator(
    task_id='kill_halted_tasks',
    python_callable=kill_halted_tasks_function,
    provide_context=True,
    dag=DAG_OBJ)

EMAIL_OR_NOT_BRANCH_OPR = ShortCircuitOperator(
...
SEND_PROCESSES_KILLED_EMAIL_OPR = EmailOperator(
